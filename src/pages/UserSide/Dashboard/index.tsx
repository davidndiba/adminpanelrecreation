...
import { Row, Col, Card, Statistic, Progress, Select, DatePicker } from 'antd';
...
const Dashboard: React.FC = () => {
  ...
  const jobStatuses = [
    { title: 'Complete', value: total_jobs, percent: (total_jobs / (total_jobs + pack_jobs_count + bulk_jobs_count)) * 100, icon: <CheckCircleOutlined style={{ color: 'green' }} /> },
    { title: 'In Progress', value: pack_jobs_count, percent: (pack_jobs_count / (total_jobs + pack_jobs_count + bulk_jobs_count)) * 100, icon: <SyncOutlined style={{ color: 'orange' }} /> },
    { title: 'Dispensed', value: bulk_jobs_count, percent: (bulk_jobs_count / (total_jobs + pack_jobs_count + bulk_jobs_count)) * 100, icon: <StopOutlined style={{ color: 'red' }} /> },
    { title: 'On Hold', value: jobs_on_hold?.length || 0, percent: jobs_on_hold ? (jobs_on_hold.length / (total_jobs + pack_jobs_count + bulk_jobs_count)) * 100 : 0, icon: <StopOutlined style={{ color: 'red' }} /> },
  ];
...
<Col span={6}>
  <ProCard title="Scheduled Jobs" bordered>
    ...
    <div style={{ display: 'flex', justifyContent: 'center', marginTop: 16 }}>
      <Progress
        type="circle"
        percent={(total_jobs / (total_jobs + pack_jobs_count + bulk_jobs_count)) * 100}
  ...
  <Statistic
    title="Pack Jobs"
    value={`${(pack_jobs_count / (pack_jobs_count + bulk_jobs_count)) * 100}%`}
  />
  <Statistic
    title="Bulk Jobs"
    value={`${(bulk_jobs_count / (pack_jobs_count + bulk_jobs_count)) * 100}%`}
  />
</div>
  </ProCard>
</Col>
...
<Col span={8}>
  ...
  <ProCard title="Upcoming Jobs" bordered style={{ maxHeight: 400, overflow: 'auto' }}>
    {upcoming_jobs && upcoming_jobs.length > 0 ? (
      upcoming_jobs.map((job: any) => (
        <Card key={job.id} style={{ marginBottom: 12 }}>
          <p> {job.schedule_date}</p>
          <p  style={{ color: '#000', fontWeight: 'bold' }} > {job.schedule_job.job_number}</p>
          <p style={{ color: 'grey', fontWeight: 'normal' }} > {job.schedule_job.description}</p>
           <p style={{ color: 'green', fontWeight: 'bold' }} >{job.schedule_job.schedule_status_id}</p>
        </Card>
      ))
    ) : (
  ...
  <ProCard title="Jobs On Hold" bordered style={{ marginTop: 16, maxHeight: 400, overflow: 'auto' }}>
    {jobs_on_hold && jobs_on_hold.length > 0 ? (
      jobs_on_hold.map((job: any) => (
        <Card key={job.id} style={{ marginBottom: 12 }}>
          <p><strong>Job Number:</strong> {job.schedule_job.job_number}</p>
          <p><strong>Description:</strong> {job.schedule_job.description}</p>
  ...
  <ProCard title="Last Month Incomplete Jobs" bordered style={{ marginTop: 16, maxHeight: 400, overflow: 'auto' }}>
    {last_month_incomplete_jobs && last_month_incomplete_jobs.length > 0 ? (
        ...
        <Col span={12}>
          <ProCard title="Scheduled Job Progress" bordered>
            {job_progress && job_progress.length > 0 ? (
              <Bar
                data={job_progress.map((progress: any) => ({ type: progress.month, value: progress.completed_count }))}
                xField="type"
                yField="value"
                seriesField="type"
